...
async function resolveMovie(id, fetchListing2, makeStream2) {
  ...
  const folders = await fetchListing2("/Stream/movie/");
...
async function resolveSeries(id, season, episode, fetchListing2, makeStream2) {
  ...
  if (!showEntries) {
    const folders = await fetchListing2("/Stream/tv/");
    const match = findFolder(folders, info.title, info.year);
...
function makeStream(file, subtitles) {
  ...
  return {
    name: file.name,
    // bold card line = full filename (Stremio & Nuvio render `name`)
    title: buildStreamTitle(meta, file.size).replace(/^ · /, ""),
    ...
    behaviorHints: {
      ...
      filename: file.name,
      videoSize: sizeToBytes(file.size)
    }
  };
}
...
var addon_default = {
  async fetch(request, env) {
    ...
    if (path === "/plugin/manifest.json" || path === "/plugin/plugin.js" || path === "/plugin/nuvio.js") {
